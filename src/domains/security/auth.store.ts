...
export const useAuthStore = create<AuthState>((set, get) => ({
  ...
  activate: async (key: string) => {
    try {
      ...
      return false;
    } catch (e: unknown) { set({ activationError: typeof e === 'string' ? e : ((e as Error)?.message || (e as { kind?: string })?.kind || 'فشل تفعيل الترخيص') }); return false; }
  },
  ...
  login: async (username: string, password: string) => {
    try {
      ...
      return true;
    } catch (e: unknown) { set({ error: typeof e === 'string' ? e : ((e as Error)?.message || (e as { kind?: string })?.kind || 'فشل تسجيل الدخول') }); return false; }
  },
