...
import { Plus, Users, User, Phone, FileText, Search } from 'lucide-react';
    ...
    <div className="p-8 overflow-auto h-full bg-slate-50 animate-fade-in">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="section-subtitle">سجل الزبائن المزمنين والوصفات الطبية</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="card-elegant p-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xl font-bold text-slate-800 tabular">{patients.length}</p>
          </div>
        </div>
        <div className="card-elegant p-4 flex items-center gap-3">
          ...
          <div>
            <p className="text-xs text-slate-500">مرضى بأرقام هواتف</p>
            <p className="text-xl font-bold text-slate-800 tabular">{patients.filter(p => p.phone).length}</p>
          </div>
        </div>
        <div className="card-elegant p-4 flex items-center gap-3">
          ...
          <div>
            <p className="text-xs text-slate-500">مرضى بملاحظات</p>
            <p className="text-xl font-bold text-slate-800 tabular">{patients.filter(p => p.notes).length}</p>
          </div>
        </div>
      </div>
      ...
      {showForm && (
        <form onSubmit={handleAdd} className="card-elegant p-6 mb-6 animate-slide-up">
          ...
          <div className="flex gap-2 mt-5">
            <button type="submit" className="btn-success">حفظ المريض</button>
            ...
            {filteredPatients.length === 0 ? (
              ...
              <tr key={p.id} className="table-row">
                ...
                <td className="p-4 text-sm text-slate-500">
                  {p.notes ? (
                    <span className="badge-warning"><FileText className="w-3 h-3" />{p.notes}</span>
                  ) : '-'}
                </td>
              </tr>
            ))}
