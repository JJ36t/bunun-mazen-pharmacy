-- Migration 0022: Materialized views + configurable session timeout
...
GROUP BY DATE_TRUNC('month', created_at), user_role ORDER BY month DESC;
...
GROUP BY created_at::date, user_role ORDER BY day DESC;
...
FROM invoice_items GROUP BY name_ar ORDER BY total_qty DESC;
...
BEGIN
    ...
    REFRESH MATERIALIZED VIEW CONCURRENTLY mv_top_medicines;
END;
$$ LANGUAGE plpgsql;
