...
use ring::{digest, hmac, constant_time};
use hex;
use bcrypt;
...
fn generate_activation_key(device_id: &str) -> String {
    let secret = "IRAQ_PHARMA_SECRET_2024_HMAC".as_bytes();
...
fn is_valid_key(device_id: &str, input_key: &str) -> bool { 
    ...
    constant_time::verify_slices_are_equal(expected_key.as_bytes(), input_key.to_uppercase().as_bytes()).is_ok()
}
